...
- Browsing the history of any local branch, remote branch, or tag by selecting it — inspection only, never a checkout: 200 topologically ordered commits at a time, with a Load More that appends the next page without duplicates or gaps. Every page of one round of paging is walked from the commit the first page started at, so a ref that moves while you read cannot open a hole in the list — a refresh is what picks the move up
...
- Deleting a local branch behind a confirmation that counts what it costs: the commits reachable from that branch and from no other ref, measured against every local branch, tag, and remote-tracking branch rather than guessed from the word "unmerged". A branch every other ref already holds says so and goes through `git branch --delete`, Git's own merged-history protection left in charge; one holding history nothing else reaches needs a default-off Force Delete ticked first, and so does the narrower refusal only Git can give. The current branch keeps the action and refuses it, saying why. A branch that moved between the confirmation and the command is refused rather than deleted under an assumption that stopped being true — and nothing about any of it reaches the remote branch, its remote-tracking ref, or the upstream configuration of anything else
- Merging another branch into the current one, from that branch's own row in the sidebar and from the Branch menu rather than the toolbar — it is infrequent and consequential, so it lives where the ref it acts on is. The confirmation names both ends in full, because direction is the thing about a merge most easily got backwards, and offers exactly Git's own three policies: Default, Fast-forward Only, and Always Create Merge Commit. Tracked local work is refused before anything runs, pointing at commit or stash rather than moving it; untracked work is left alone unless Git reports a path it would have written over, and then the refusal names those paths. There is no Smart Merge, no automatic stash, no squash merge, and nothing that skips the commit or a hook
- A merge that stops at a conflict is a state to work in, not an error to dismiss: the conflicted paths sort to the top of the changes list, the state survives a refresh and a relaunch, and the version choices are labelled with the real branch or commit each side came from instead of ours and theirs. Open in Default Editor is there for the lines only a person can settle, and Mark as Resolved stages what the file now holds without claiming to have merged it. Continue stays unavailable until every unmerged path is resolved, and Abort is Git's own rollback
- Saving uncommitted work as a real stash, through one sheet with an optional message and the two exceptions to what `git stash push` saves by default — Keep Staged Changes and Include Untracked Files, both unchecked, because each is a departure from what stashing ordinarily means. Ignored files are never included: the command is `--include-untracked` and never `--all`, so build output and secrets stay where they are whichever way the options are set. A repository with nothing eligible to save says so instead of running a command Git ends successfully having created nothing, and a working tree whose only work is untracked names the option that would save it rather than sitting refused
- Reading a stash back: the entries Git holds, each at the address Git addresses it by, with the description Git stored, who saved it and when, and whether it carries files Git was not tracking. Selecting one lists the paths it saved and reads that path's patch under the same size limits every other diff has — inspection only, with no Stage action beside it, because a stash is content Git holds outside the working tree. A file that was untracked when the stash was saved is compared against nothing, which is what it was, rather than against a commit that never held it
...
- Fetch Remotes in the Remotes section, which downloads every eligible remote's branches and removes the remote-tracking branches those remotes no longer have — the ordinary fetch deliberately does not, so a branch someone deleted after merging a pull request has somewhere to be cleaned up from without reaching for a terminal. It removes nothing outside `refs/remotes/`, and that holds even in a repository configured with `fetch.pruneTags` or a `+refs/tags/*` refspec: it names its own branch refspec rather than pruning within whatever the remote was configured to fetch. No tag, no local branch, and no branch on any remote is ever deleted, and a local branch whose upstream just went away says so rather than reporting itself level with something that is no longer there
...
- Choosing the interface language yourself, in a Settings window the app menu opens with ⌘, — English, 日本語, 한국어, 简体中文, 繁體中文, or whatever the system asks for, each named in itself. The choice is kept where macOS's own per-app language setting keeps it, so the two can never disagree, and it takes effect at the next launch — which the window offers to do for you, unless a Git command is still running. It is reachable with no repository open and with Git missing entirely, which are the two states a language you cannot read is hardest to escape from. Git's own output stays in the system language
...
Still missing: hunk staging, applying or removing a stash, and rebase. The interface was settled first as the design target, and the backend is working its way up to it.
...
- Xcode 27.0 with the macOS 27.0 SDK and Swift 6.4 to build
